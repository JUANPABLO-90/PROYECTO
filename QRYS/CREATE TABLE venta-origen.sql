...
CREATE INDEX venta__IDX ON venta_origen
  (
    id ASC
  ) ;

CREATE INDEX venta_IDX ON venta_origen
  (
    fecha ASC ,
    hora ASC
  ) ;
...
ALTER TABLE venta_origen ADD CONSTRAINT venta_serie_UN UNIQUE ( serie ) ;
ALTER TABLE venta_origen ADD CONSTRAINT venta_PK PRIMARY KEY ( id ) ;
